...
import { getContentBySlug, getContentItems } from "../app/features/content/get";
...
export const getStaticPaths: GetStaticPaths<PathParams> = async () => {
  ...
  const slugs = contentItems
    ...
    .map((article: Content) => article.slug);

  const paths = slugs.map((slug) => ({ params: { "article-slug": slug } }));
...
export const getStaticProps: GetStaticProps<ArticleProps, PathParams> = async ({
  ...
  const content = await getContentBySlug(params["article-slug"], "article");
  if (!content) {
    return <Error statusCode={404} />;
  }
